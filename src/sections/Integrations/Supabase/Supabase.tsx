...
import { Box, Stack, Typography, Paper, Divider, Chip, Button, Link as MUILink } from '@mui/material';
...
export default function SupabaseIntegration() {
  ...
  const tb = useTranslations('billing');      // لعناوين السايدبار الموجودة
      ...
      <Stack direction="row" gap={2} sx={{ position: 'relative', zIndex: 1, p: { xs: 2, md: 3 }, maxWidth: 1280, mx: 'auto' }}>
        ...
        <Stack spacing={2} sx={{ flex: 1 }}>
          ...
          <Card>
            <Stack direction="row" alignItems="center" justifyContent="space-between">
              <Stack>
                <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: .5 }}>
                  <Typography sx={{ fontWeight: 800 }}>{t('orgs')}</Typography>
                  <Chip label={t('admin')} size="small" />
                </Stack>
                <Typography sx={{ opacity: .85 }}>{t('orgs_desc')}</Typography>
              </Stack>

              <Button
                component={Link as any}
                href="/integrations/supabase/organizations"
                variant="outlined"
            ...
            <Divider sx={{ borderColor: palette.line, mt: 2 }} />
          </Card>
        </Stack>
      </Stack>
    </Box>
